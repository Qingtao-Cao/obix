...
static Periodic_Task *periodicTask_create(
    ...
    periodicTask_resetExecTime(thread, ptask, true);
...
static void periodicTask_execute(Task_Thread *thread, Periodic_Task *ptask) {
    ...
    periodicTask_generateNextExecTime(thread, ptask, false);
}
...
int ptask_reschedule(Task_Thread *thread, int taskId, long period, int executeTimes, bool add) {
    ...
    if (add) {
        ...
        periodicTask_setPeriod(ptask, period, executeTimes);
        periodicTask_resetExecTime(thread, ptask, false);
    }
 ...
 *
 * NOTE: the wait parameter must be false under any of following
